...
export class VirtualAccountService {
    ...
    private async provision(userId: string) {
        ...
        if (this.flw.isLive && !user.kyc?.bvnVerifiedAt) {
            throw new BadRequestException(
                'BVN must be verified before creating a live virtual account.',
            );
        }
    ...
    private resolveProviderAccountActive(data: { is_active?: boolean; response_message?: string }): boolean {
