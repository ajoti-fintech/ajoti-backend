...
describe('Wallet API (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // TODO: Get real auth token from your auth system
    // For now, we'll mock it in the request headers
    ...
    it('should return wallet statistics', async () => {
      ...
      expect(response.body).toMatchObject({
        ...
        data: {
          ...
          lastTransaction: expect.toBeOneOf([null, expect.any(String)]),
        },
      });
    });
  });
  ...
  describe('GET /wallet/buckets', () => {
    it('should return wallet buckets', async () => {
      ...
      // Verify bucket types
      const bucketTypes = response.body.data.map((b) => b.bucketType);
  ...
  describe('Admin Endpoints', () => {
    describe('GET /admin/wallet/user/:userId', () => {
      it('should get wallet by userId (admin only)', async () => {
        // TODO: Add admin auth token
        const response = await request(app.getHttpServer())
          .get(`/admin/wallet/user/${mockUserId}`)
          .set('Authorization', `Bearer ${authToken}`) // Use admin token
          .expect(200);
