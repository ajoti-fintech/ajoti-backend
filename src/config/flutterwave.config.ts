...
export const flutterwaveConfig = registerAs('flutterwave', () => {
  ...
  const isLive =
    explicitEnv === 'live' || (explicitEnv !== 'test' && process.env.NODE_ENV === 'production');
  const bypassRequested = process.env.BYPASS_WEBHOOK_VERIFICATION === 'true';

  const secretKey = isLive ? process.env.FLW_SECRET_KEY_LIVE : process.env.FLW_SECRET_KEY_TEST;

  const publicKey = isLive ? process.env.FLW_PUBLIC_KEY_LIVE : process.env.FLW_PUBLIC_KEY_TEST;
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    if (bypassRequested) {
      throw new Error('BYPASS_WEBHOOK_VERIFICATION must be false in production.');
    }
  }
  ...
  return {
    ...
    bypassWebhookVerification: process.env.NODE_ENV === 'production' ? false : bypassRequested,
